...
/*   Updated: 2023/07/13 09:53:14 by lsordo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	isScientific(std::string str) {
	...
	double result;
	iss >> result;
	std::string::size_type	posE = str.find('e');
	if (posE != std::string::npos && result)
		return true;
	return false;
}

bool	isSuspiscious(std::string str) {
	int	flag = 0;
...
std::string	formatOutputDouble(double result) {
	...
	if (result == INFINITY || result == -INFINITY || result == NAN)
		return strValue;
	else if (result < std::numeric_limits<double>::min() && !isZero(strValue))
...
std::string	formatOutputFloat(double result) {
	...
	if (result == INFINITY || result == -INFINITY)
		return (strValue + "f");
	else if (result < std::numeric_limits<float>::min() && !isZero(strValue))
		return ERR_CONVERTIBLE;
	std::string::size_type	posE = strValue.find('e');
	if (posE != std::string::npos)
		return (strValue + "f");
...
void	ScalarConverter::convert(std::string str) {
	...
	else if ((!isScientific(str) && isSuspiscious(str)) || (!result && str != "0" && str.length() > 1)) {
