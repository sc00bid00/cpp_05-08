...
/*   Updated: 2023/07/22 15:58:31 by lsordo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void) {
	{
		srand(time(NULL));
		try {
			/* default constructor for int array size 3 */
			...
			/* assignemt operator on int array */
			...
			std::cout << BYELLOW << "Complex Awesome array" << RESET << std::endl;
	...
	{
		try {
			std::cout << BYELLOW << "Awesome array (i.e. complex array)" << RESET << std::endl;
			/* default constructor on complex array */
			Array<Awesome> awesomeArray(3);
			std::cout << "Array size is : " << awesomeArray.size() << std::endl;
			for (int i = 0; i < 3; i++) {
				std::cout << awesomeArray[i] << std::endl;
			}
		}
		catch (std::exception &e) {
			std::cerr << BRED << e.what() << RESET << std::endl;
	...
	{
		try {
			std::cout << BYELLOW << "String array" << RESET << std::endl;
